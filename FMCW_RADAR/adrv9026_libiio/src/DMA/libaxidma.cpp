...
#include "../../include/DMA/libaxidma.h"          // Local definitions
#include "../../include/DMA/axidma_ioctl.h"       // The IOCTL interface to AXI DMA
        ...
        struct axidma_chan *channels, struct axidma_num_channels *num_chan)
{
    ...
    dev->channels = (dma_channel_t *)malloc(num_chan->num_channels * sizeof(dev->channels[0]));
    ...
    dev->dma_tx_chans.data = (int *)malloc(num_chan->num_dma_tx_channels *
            sizeof(dev->dma_tx_chans.data[0]));
    if (dev->dma_tx_chans.data == NULL) {
        free(dev->channels);
        return -ENOMEM;
    }
    dev->dma_rx_chans.data = (int *)malloc(num_chan->num_dma_rx_channels *
    ...
    dev->vdma_rx_chans.data = (int *)malloc(num_chan->num_vdma_rx_channels *
...
static int probe_channels(axidma_dev_t dev)
{
    ...
    channels = (axidma_chan *)malloc(num_chan.num_channels * sizeof(channels[0]));
    if (channels == NULL) {
        return -ENOMEM;
    }
