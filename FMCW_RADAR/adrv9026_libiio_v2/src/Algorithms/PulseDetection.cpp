...
void PulseDetection_LowPulse_sub_thread(bool & found, int & selected, char * address, int start_index, int end_index, int b_i, int l_i, float noise) {
  
  for (int pulse_index = start_index;pulse_index < end_index; pulse_index += b_i) { 
    short real_4, real_3, real_2, real_1, real_0, real_p1;
    short imag_4, imag_3, imag_2, imag_1, imag_0, imag_p1;
    
    std::memcpy(&real_4, address +  pulse_index - 4 * b_i + l_i, 2);
    std::memcpy(&imag_4, address +  pulse_index - 4 * b_i + l_i + 2, 2);
    std::memcpy(&real_3, address +  pulse_index - 3 * b_i + l_i, 2);
    std::memcpy(&imag_3, address +  pulse_index - 3 * b_i + l_i + 2, 2);
    std::memcpy(&real_2, address +  pulse_index - 2 * b_i + l_i, 2);
    std::memcpy(&imag_2, address +  pulse_index - 2 * b_i + l_i + 2, 2);
    std::memcpy(&real_1, address +  pulse_index - 1 * b_i + l_i, 2);
    std::memcpy(&imag_1, address +  pulse_index - 1 * b_i + l_i + 2, 2);
    std::memcpy(&real_0, address +  pulse_index - 0 * b_i + l_i, 2);
    std::memcpy(&imag_0, address +  pulse_index - 0 * b_i + l_i + 2, 2);
    std::memcpy(&real_p1, address + pulse_index + 1 * b_i + l_i, 2);
    std::memcpy(&imag_p1, address + pulse_index + 1 * b_i + l_i + 2, 2);
...
int PulseDetection_LowPulse_Multi(iio_buffer * rx_buf, int loop_chan, int signal_length, int enable_chans) {
  int pulse_index = 4 * enable_chans * sizeof(std::complex<short>), // (IQ0 4 IQ1 8 IQ2 12 IQ3 16) * 4
    ...
    loop_interval = loop_chan * sizeof(std::complex<short>),
    expected_length = signal_length * byte_interval * 4;
  float noise = 500.0;
  bool _found_ = false;
  ...
  int _selected_[8] = {INT_MAX , INT_MAX , INT_MAX , INT_MAX , INT_MAX , INT_MAX , INT_MAX , INT_MAX };
  int length = expected_length / 8;
  for (int ii = 0; ii < 8; ii++) {
    _threads_[ii] = std::thread(PulseDetection_LowPulse_sub_thread, std::ref(_found_), std::ref(_selected_[ii]), start_address, 
          pulse_index + ii * length, pulse_index + (ii+1) * length, byte_interval, loop_interval, noise);
  }
  
  pulse_index = INT_MAX;
  for (int ii = 0; ii < 8; ii++) _threads_[ii].join();
  for (int ii = 0; ii < 8; ii++) pulse_index = std::min(pulse_index, _selected_[ii]);
