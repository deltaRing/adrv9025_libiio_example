...
Eigen::ArrayXXcf MIMO_LFM(int channel_num, MIMO_LFM_conf & conf){
  ...
  Eigen::ArrayXXcf lfm(1, PRI_num);
  Eigen::ArrayXf t_;
  if (conf._use_half_band_) t_ = Eigen::ArrayXf::LinSpaced(tp_num, 0.0, tp);
  else t_ = Eigen::ArrayXf::LinSpaced(tp_num, -tp / 2.0, tp / 2.0);
  ...
  conf.FFT_FMCW       = Get_FMCW_FFT(lfm, _RFFT_NUM_);
