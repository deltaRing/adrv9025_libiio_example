...
void parse_config_file(std::unordered_map<std::string, std::vector<float>> & confData, 
  ...
  struct CancellationParam & cparams,
  bool ec[4]) {
  ...
  for (const auto& [key, values] : confData) {
    ...
    if (key == std::string("mimo")) {
      ...
      for (float v:values) {
        if (index == 0) cparams.expand_num = v;
        if (index == 1) cparams.init_cancel_num = v;
        if (index == 2) cparams.separate_num = v;
        if (index == 3) cparams.cancel_weight = v;
        index++;
      }
