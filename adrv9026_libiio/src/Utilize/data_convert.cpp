...
void GetData16(std::complex<int16_t *> ** data, int enable_channel_num, int sample_num, struct iio_buffer * rx_buf) {
  ...
  int ii = 0;
  char * address = (char *)iio_buffer_start(rx_buf);
  p_end = (char *)iio_buffer_end(rx_buf);
  
  for (p_dat = (char *)iio_buffer_start(rx_buf); p_dat < p_end; p_dat += p_inc){
    if (enable_channel_num >= 1){
      data[0][ii] = std::complex<int16_t *> {(int16_t *)address + ii * p_inc, (int16_t *)address + 2 + ii * p_inc};
    }
    if (enable_channel_num >= 2){
      data[1][ii] = std::complex<int16_t *> {(int16_t *)address + 4 + ii * p_inc, (int16_t *)address + 6 + ii * p_inc}; 
    }
    if (enable_channel_num >= 3){
      data[2][ii] = std::complex<int16_t *> {(int16_t *)address + 8 + ii * p_inc, (int16_t *)address + 10 + ii * p_inc};
    }
    if (enable_channel_num >= 4){
      data[3][ii] = std::complex<int16_t *> {(int16_t *)address + 12 + ii * p_inc, (int16_t *)address + 14 + ii * p_inc};   
    }
    ii++;
  }
}
